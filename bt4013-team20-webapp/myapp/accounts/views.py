...
def get_supabase_client(require_service_role: bool = False) -> Client:
    """Return a Supabase client configured for server-side writes when needed.

    - When require_service_role is True, use SUPABASE_SERVICE_ROLE_KEY to ensure
      upserts/updates succeed under RLS. Falls back to SUPABASE_KEY with a warning
      if the service key is not available.
    """
    ...
    if not url or not key:
        # Fail fast with a clear error so deployments surface misconfiguration early.
...
class SimpleLoginView(LoginView):
    ...
    def get_success_url(self):
        ...
        if redirect_to:
            return redirect_to
        # Default: send users who logged in from the login page to the chat page when available.
        try:
...
def profile_view(request):
    ...
    Mirrors the required fields from signup, validates them server-side, persists
    to the local database, and mirrors updates to Supabase on a best-effort basis.
    """
    user = request.user
    ...
    if request.method == 'POST':
        ...
        # Years of experience (required) — accept decimals like 0.5
        ...
        # Work experiences parse and validation
        work_exps = []
        try:
        ...
        print("Profile POST data:", request.POST, file=sys.stderr)

        # Debug: Log extracted and parsed values
        print("Parsed email:", email, file=sys.stderr)
        print("Parsed job_title:", job_title, file=sys.stderr)
        print("Parsed skills:", skills, file=sys.stderr)
        print("Parsed median_salary:", median_salary, file=sys.stderr)
        print("Parsed years_experience:", years_experience, file=sys.stderr)
        print("Parsed currency:", currency, file=sys.stderr)
        print("Parsed work_experiences:", work_exps, file=sys.stderr)
        ...
        # Update user email (also username for authentication)
        ...
        # Mirror profile update to Supabase (best-effort; do not block the user)
        try:
            ...
            # Debug: Log Supabase payload and response
            print("Supabase payload:", payload, file=sys.stderr)
            if resp is not None:
                print("Supabase response - status:", getattr(resp, 'status_code', None), file=sys.stderr)
                print("Supabase response - data:", getattr(resp, 'data', None), file=sys.stderr)
                print("Supabase response - error:", getattr(resp, 'error', None), file=sys.stderr)
            else:
                print("Supabase response is None", file=sys.stderr)

            # Interpret response and log server-side; UI always shows a generic success below
            if resp is not None:
        ...
        # Show a simple success message; the page will redirect to GET /profile to display it
...
def settings_view(request):
    ...
    if request.method == 'POST':
        print("Settings POST data:", request.POST, file=sys.stderr)
