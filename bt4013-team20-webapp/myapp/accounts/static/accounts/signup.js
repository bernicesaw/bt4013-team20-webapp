...
function setupPasswordValidation(opts) {
  ...
  // If there is no password input on this page, do not bind any validation to the form.
  if (!input) {
    return;
  }
  ...
  if (form) {
    form.addEventListener('submit', (e) => {
      try {
        ...
        console.warn('Failed to write skillsHidden before submit', err);
      }
      try {
        updateWorkHidden();
      } catch (err) {
        console.warn('updateWorkHidden threw during submit', err);
      }
      // Debug: surface values so developers can see why a submit may be blocked
      try {
        console.log('Profile form submit - skillsHidden:', skillsHidden ? skillsHidden.value : '(missing)');
        console.log('Profile form submit - workExperiencesHidden:', workExpsHidden ? workExpsHidden.value : '(missing)');
      } catch (err) {
        console.log('Error logging submit debug info', err);
      }
      ...
      if (!ok) {
        // Log why client-side validation is blocking submission
        console.log('Profile form blocked by client-side validation', { workOk, skillsCount: skills ? skills.length : 0 });
        e.preventDefault();
      }
    });
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  try {
    // Only binds if the password input exists (signup page). On other pages (e.g., profile), this becomes a no-op.
